...
def film_report(model, data, groups: dict[str, float]) -> dict:
    ...
    cfg = model.cfg
    length_um = float(cfg.scales.L_ref_um)
    x_root, _ = model.film_root()
    dry_threshold = DRY_REPORT_FACTOR * groups["film_dryout_star"]
    fractions = torch.tensor(_stations(PROFILE_STATIONS * 4), dtype=torch.float32)

    d = data.domain
    ys = torch.linspace(d.y_min, d.y_max, PARTITION_GRID)
    xs = torch.linspace(d.x_min, d.x_max, PARTITION_GRID)
    grid = torch.cartesian_prod(xs, ys)

    per_frame = []
    for row, frame in enumerate(data.frame_numbers):
        t_value = float(data.t[row])
        times = torch.tensor([[t_value]])
        with torch.no_grad():
            nose = model.apex(times)[0, 0]
            x = x_root + (nose - x_root) * fractions.reshape(-1, 1)
            delta = model.film(x, torch.full_like(x, t_value)).squeeze(1)
            points = torch.cat([grid, torch.full((grid.shape[0], 1), t_value)], dim=1)
            alpha = model.alpha(points).squeeze(1)
            source = model.source(points).squeeze(1).clamp(min=0.0)
        front = model.front(times, n_body=64, n_cap=8)
        pressure = film_surface_pressure(model, front, groups).squeeze(1)
        body = front.on_cap.squeeze(1) == 0
        peak = int(pressure[body].argmax())
        peak_x = float(front.points[body, 0].detach()[peak])

        interior = (source * (alpha > 0.95)).sum()
        total = source.sum().clamp(min=1e-12)
        per_frame.append(
            {
                "frame": int(frame),
                "delta_um": [float(v) * length_um for v in delta],
                "dry_fraction": float((delta <= dry_threshold).float().mean()),
                "film_evaporation_fraction": float(interior / total),
                "film_pressure_peak_fraction": (peak_x - x_root) / (float(nose) - x_root),
            }
        )

    last = per_frame[-1]
    return {
        "stations": [float(v) for v in fractions],
