...
class _Tensors(NamedTuple):
    ...
    y_roi_top: int
    # Per-frame smooth interface curve in [x*, y*], (rows, points, 2), drawn on
    # the overlay directly so it is not re-quantised off the alpha raster.
    interface_star: np.ndarray
...
def _load(settings: Settings, dataset: str) -> _Tensors | None:
    ...
    with np.load(path) as data:
        ...
        return _Tensors(
            ...
            interface_star=data["interface_star"],
        )
    ...
    return QcInterface(
        ...
        frames=[
            QcInterfaceFrame(
                ...
                rings=[_closed_ring(tensors.interface_star[i])],
            )
...
def _closed_ring(curve: np.ndarray) -> list[list[float]]:
    """The stored interface curve as one closed [x*, y*] ring (loop repeats the
    first point at the end so it is a proper polygon). It is already the smooth
    interface the tensors were built from -- drawn as-is, not re-traced."""
    ring = np.round(curve, 4)
    return np.vstack([ring, ring[:1]]).tolist()
