...
def is_valid_dataset_id(dataset: str) -> bool:
    return bool(_DATASET_RE.match(dataset))
...
def _raw_dir(settings: Settings, dataset: str) -> Path | None:
    ...
    if not is_valid_dataset_id(dataset):
        return None
    raw_root = settings.data_raw_dir.resolve()
    raw_dir = (raw_root / dataset).resolve()
    return raw_dir if raw_dir.is_relative_to(raw_root) else None


def _processed_dir(settings: Settings, dataset: str) -> Path:
    return settings.repo_root / "data" / "processed" / dataset
...
def _is_processed(settings: Settings, dataset: str) -> bool:
    return (_processed_dir(settings, dataset) / "tensors.npz").is_file()
...
def list_datasets(settings: Settings) -> list[DatasetSummary]:
    ...
    for d in sorted(raw_root.iterdir()):
        if not d.is_dir() or not is_valid_dataset_id(d.name):
            continue
        summaries.append(
            DatasetSummary(
                id=d.name,
                n_frames=_count_frames(d),
                processed=_is_processed(settings, d.name),
            )
        )
    return summaries
...
def qc_path(settings: Settings, dataset: str) -> Path | None:
    ...
    qc = (processed_root / dataset / "qc_preprocess.png").resolve()
    if not qc.is_relative_to(processed_root) or not qc.is_file():
        return None
    return qc
...
def save_frames(settings: Settings, dataset: str, frames: list[bytes]) -> int:
    ...
    Server-generated names, TIFF magic + size + count checks (SECURITY.md §2).
    """
    raw_dir = _raw_dir(settings, dataset)
    if raw_dir is None:
    ...
    for i, data in enumerate(frames, start=1):
        ...
        if not data.startswith(TIFF_MAGIC):
            raise UploadError(f"frame {i} is not a TIFF image")
