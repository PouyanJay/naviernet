...
export function ReconstructionPanel({ runId }: { runId: string }) {
  ...
  if (load.status === "unavailable") {
    return (
      <p className="state-note">
        No trained model to reconstruct from — train this run in the Solver first.
      </p>
    );
  }
  if (load.status === "error") {
    return (
      <p className="state-note error" role="alert">
        Could not load the reconstruction: {load.message}
      </p>
    );
  }
  ...
  const ratio =
    cameraInstants > 1 ? Math.round(data.frames.length / (cameraInstants - 1)) : null;
  return (
    <div>
      <ReconstructionViewport data={data} />
      <p className="figcap">
        <b>Figure 1.</b> Continuous PINN interface reconstruction — {data.frames.length}{" "}
        instants{ratio != null && ratio > 1 && <> · ~{ratio}× finer than the camera</>}. Dashed
        contours are the measured interface at camera instants.
      </p>
    </div>
  );
}
