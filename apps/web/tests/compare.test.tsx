...
const CANDIDATES = [
  { id: "sweep-1-s0", dataset: "highest_t", status: "trained", steps: 40, iou_holdout: 0.62 },
  { id: "sweep-1-s1", dataset: "highest_t", status: "trained", steps: 40, iou_holdout: 0.64 },
...
function stubApi() {
  vi.stubGlobal(
    ...
    vi.fn(async (url: RequestInfo | URL) => {
      ...
      if (detailMatch) return json(detailFor(decodeURIComponent(detailMatch[1])));
...
describe("ComparePanel", () => {
  it("asks for a second run before comparing", () => {
    ...
    expect(screen.getByText("Select at least two runs to compare them.")).toBeInTheDocument();
  });
  ...
  it("renders overlaid loss + IoU charts and the metrics table for a selection", async () => {
    ...
    await waitFor(() =>
      expect(container.querySelectorAll("path.chart-line.series-0")).toHaveLength(2),
    );
    ...
    const pressed = container.querySelectorAll('.compare-choice[aria-pressed="true"]');
  ...
  it("caps the selection at four runs", () => {
    ...
    render(
      <ComparePanel
        candidates={[
          ...CANDIDATES,
          { id: "r4", dataset: null, status: "trained", steps: 1, iou_holdout: null },
          { id: "r5", dataset: null, status: "trained", steps: 1, iou_holdout: null },
        ]}
      />,
    );
    ...
    const pressed = document.querySelectorAll('.compare-choice[aria-pressed="true"]');
