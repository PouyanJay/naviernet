...
test("renders the shell and a real run from the API", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Results & validation" }),
  ).toBeVisible();
...
test("physics & model view shows equations and the live topology", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(
    page.getByRole("heading", { name: "Governing equations" }),
  ).toBeVisible();
  await expect(page.locator(".katex").first()).toBeVisible(); // KaTeX rendered
  await expect(
    page.getByRole("heading", { name: "Model topology · live" }),
  ).toBeVisible();
